...
#' @param fixed_inf_hists (optional) Data frame with columns `individual`, `time`, and `value`, giving infection states that should be fixed during the MCMC run. See the [advanced features vignette](https://seroanalytics.github.io/serosolver/articles/advanced_features.html).
...
#' @param measurement_bias optional NULL. A data frame mapping each `biomarker_id` and `biomarker_group` combination to the `rho_index` of the measurement-shift parameter that it uses. See the [advanced features vignette](https://seroanalytics.github.io/serosolver/articles/advanced_features.html).
...
#' @param start_level either `"none"` or a data frame giving the starting biomarker level for each individual, `biomarker_group`, and `biomarker_id` combination. With `"none"`, starting levels are assumed to be 0. See the [advanced features vignette](https://seroanalytics.github.io/serosolver/articles/advanced_features.html).
...
#' @param exponential_waning Deprecated. If TRUE, assumes exponential waning of antibody levels rather than linear waning, and changes the cross-reactivity model from a linear decline with antigenic distance to an exponential decline. Prefer a fixed `exponential_waning` row in `par_tab`, with `values = 1` and `par_type = 0`. See the [advanced features vignette](https://seroanalytics.github.io/serosolver/articles/advanced_features.html).
