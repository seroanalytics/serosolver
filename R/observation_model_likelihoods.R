...
r_likelihood <- function(expected, data, theta, expected_indices = NULL, measurement_shifts = NULL) {
  ...
  small_i <- data < theta["min_measurement"] + 1
  rest_i <- data >= theta["min_measurement"] + 1 & data < theta["max_measurement"]
  
  liks[large_i] <- pnorm(theta["max_measurement"], expected[large_i], theta["obs_sd"], lower.tail = FALSE, log.p = TRUE)
  liks[small_i] <- pnorm((theta["min_measurement"]+1), expected[small_i], theta["obs_sd"], lower.tail = TRUE, log.p = TRUE)
