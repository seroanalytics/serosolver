# Modified by an AI assistant on 2026-09-18 using GPT-5. Updated the example
# antibody-data documentation to include its population-group column.
...
#' @format A data frame with 6000 rows (each corresponding to an antibody measurement) and 8 variables:
#' \describe{
...
#'     \item{population_group}{one-based integer ID identifying the population group for each individual.}
#' }
#' @family example_data
"example_antibody_data"

#' Example longitudinal antibody data
#'
#' Example antibody data for a longitudinal study design, generated by \code{simulate_data}.
#' The data contain repeated samples from the same individuals over time and use one biomarker group.
#' @docType data
#' @usage data(example_longitudinal_antibody_data)
#' @format A data frame with 500 rows (each corresponding to an antibody measurement) and 7 variables: `individual`, `sample_time`, `birth`, `repeat_number`, `biomarker_id`, `biomarker_group`, and `measurement`.
#' @family example_data
"example_longitudinal_antibody_data"

#' Example antibody data with multiple biomarker groups
#'
#' Example antibody data generated by \code{simulate_data} with two biomarker groups. The two groups represent different observation types measured for the same individuals and sampling times.
#' @docType data
#' @usage data(example_multiple_biomarker_groups_data)
#' @format A data frame with 1,000 rows (each corresponding to an antibody measurement) and 7 variables: `individual`, `sample_time`, `birth`, `repeat_number`, `biomarker_id`, `biomarker_group`, and `measurement`.
#' @family example_data
"example_multiple_biomarker_groups_data"
