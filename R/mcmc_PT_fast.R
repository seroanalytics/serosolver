...
#' @param seed random seed to set for the MCMC, if desired
#' @details
...
run_MCMC_pt_fast <- function(par_tab,
  ...
  temperatures <- mcmc_pars[["temperature"]]
  parallel_tempering_iter <- mcmc_pars[["parallel_tempering_iter"]]
  ###################################################################
  ...
  ## startinf pars and step sizes for each rung seperately -
  ...
  message(cat("startinf theta posterior probability: ", posterior, sep = "\t"))
  ...
  ## Write startinf conditions to file
  ...
  run_multiple_iter <- function(tmp_list) {
    start_i <- tmp_list$i
    for (i in start_i:(start_i + n_iter)) {
      tmp_list <- run_MCMC_single_iter(
        i, tmp_list$par_i,
  ...
  run_MCMC_single_iter <- lapply(
    1:length(temperatures),
    function(x) create_run_MCMC_single_iter_fn_fast(
        ...
        extra_probabilities,
        proposal_gibbs,
        alpha, beta, hist_sample_prob,
        n_infs_vec,
        swap_propn, move_size, n_alive,
        switch_sample,
        hist_switch_prob,
        year_swap_propn
      )
