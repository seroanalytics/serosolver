...
setup_infection_histories_old <- function(titre_dat, strain_isolation_times, sample_prob, titre_cutoff=3){
    ...
    samplingTimes <- unique(titre_dat$samples)
    ...
    index <- 1
    ## For each individual
    tmp_inf_hist_indiv <- matrix(0,nrow=length(samplingTimes),ncol=n_strain)
    for(indiv in unique(titre_dat$individual)){
        ...
        for(strain in tmp_strains){
            tmpTitre <- max(titre_dat[titre_dat$virus == strain &
...
#' @param strain_isolation_times the vector of times to sample infections for
