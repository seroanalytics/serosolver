...
#' @param measurement_bias if not NULL, then use these indices to specify which measurement bias parameter index corresponds to which time. See the [advanced features vignette](ADVANCED_FEATURES_VIGNETTE_LINK) for this optional workflow.
...
#' @param function_type integer specifying which version of this function to use. Specify 1 to give a posterior solving function; 2 to give the gibbs sampler for infection history proposals; otherwise just solves the antibody model and returns predicted antibody levels. NOTE that this is not the same as the attack rate prior argument, \code{version}!
...
#' @param demographics if not NULL, then a tibble for each individual (1:n_indiv) giving demographic variable entries. Most importantly must include "birth" as the birth time. This is used if, for example, you have a stratification grouping in `par_tab`. See the [demographics and covariates vignette](DEMOGRAPHICS_VIGNETTE_LINK) for this optional workflow.
