...
#' @export
infection_history_symmetric <- function(new_inf_hist, sampled_indivs, age_mask, strain_mask,moveSizes, n_infs, rand_ns){
...
infection_history_betabinom <- function(new_inf_hist, sampled_indivs, age_mask,strain_mask, move_sizes, alpha, beta){
    ...
    for(indiv in sampled_indivs){
        ...
        #newInf[indiv,age_mask[indiv]:ncol(new_inf_hist)]=x
...
infection_history_betabinom_group <- function(new_inf_hist, sampled_indivs, age_mask,strain_mask, move_sizes, n_infs, alpha, beta){
    newInf <- new_inf_hist
    for(indiv in sampled_indivs){
       # x <- newInf[indiv,age_mask[indiv]:ncol(new_inf_hist)]
      x <- new_inf_hist[indiv, age_mask[indiv]:strain_mask[indiv]]
...
#' @export
inf_hist_prob_lambda <- function(newInf, sampled_indivs, age_mask,strain_mask, n_infs, lambdas){
    #ks <- rpois(length(sampled_indivs),n_infs)
    for(i in 1:length(sampled_indivs)){
        indiv <- sampled_indivs[i]
...
#' @export
infection_history_proposal <-function(newInfectionHistories,sampled_indivs,strain_isolation_times,age_mask, strain_mask,n_infs){
