...
simulate_group <- function(n_indiv, theta, infection_histories,
    ...
    for(i in 1:n_indiv){
        ...
        y <- as.data.frame(simulate_individual(theta, infection_histories[i,],
...
#' @param sample_times the vector of times at which samples are taken
#' @param dataIndices see \code{\link{create_posterior_func}}
#' @param strain_isolation_times see \code{\link{create_posterior_func}}
#' @param virusIndices see \code{\link{create_posterior_func}}
#' @param strain_isolation_times the vector of strain circulation times (ie. possible infection times)
#' @param antigenic_map_long the collapsed antigenic map for long term cross reactivity, after multiplying by sigma1
#' @param antigenic_map_short the collapsed antigenic map for short term cross reactivity, after multiplying by sigma2
...
simulate_individual <- function(theta,
    ...
    if(is.null(mu_indices)){    
        titres <- titre_data_individual(theta, infection_history, strains, seq_along(strains)-1, sampling_times,
                                        dataIndices, match(strain_isolation_times, strains)-1,
                                        antigenic_map_long, antigenic_map_short, numberStrains)
    } else {
        titres <- titre_data_individual_mus(theta, mus, infection_history, strains, seq_along(strains)-1,
                                        mu_indices, sampling_times,
                                        dataIndices, match(strain_isolation_times, strains)-1,
                                        antigenic_map_long, antigenic_map_short, numberStrains)
    }
    titres <- rep(titres, each=repeats)
    sampling_times <- rep(sampling_times, dataIndices)
...
simulate_infection_histories <- function(p_inf, strain_isolation_times, sampling_times, ages){
    ...
    ## Simulate attack rates
    attack_rates <- p_inf
    #attack_rates <- rlnorm(length(pInf), meanlog=log(pInf)- infSD^2/2,sdlog=infSD)
...
simulate_data <- function(par_tab, group=1,n_indiv,buckets=12,
                          ...
                          antigenic_map,
                          sampleSensoring=0, titre_sensoring=0,
    ...
    lambda_pars <- par_tab[par_tab$identity == 2,"values"]
    ...
    ## Simulate infection histories    
    tmp <- simulate_infection_histories(attack_rates, simInfPars["logSD"], strain_isolation_times,
                                        sampling_times, ages)
    ...
    ## Simulate titre data
    y <- simulate_group(n_indiv, pars, infection_history, strain_isolation_times, sampling_times,
