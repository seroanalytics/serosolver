	     ...
	     double *theta, int *inputtestyr)
{
  ...
  // printf("Mu: %f; wane: %f; t2: %f; mu2: %f",mu,wane,T_2,mu2);
...
#define MAX(a,b) ((a) < (b) ? (b) : (a)) // define MAX function for use later
  ...
  for (k=0; k<nsamp; k++){ // Iterate over samples tested against
    ...
    /* JAMES HAY COMMENT
       The following 3 loops do not look to be dependent on k
       so could be moved outside of the loop
    */
