...
serosolver(par_tab, antibody_data %>% mutate(birth = 2010), example_antigenic_map,
...
chains <- load_mcmc_chains(location=getwd(),par_tab=par_tab,burnin = 10000,unfixed=FALSE)
plot_model_fits(chain = chains$theta_chain,
                infection_histories = chains$inf_chain,
                known_infection_history = NULL,
                antibody_data = antibody_data %>% mutate(birth = 2010),individuals=1:5,
                antigenic_map=example_antigenic_map,
                nsamp=100,
                par_tab=par_tab,
                orientation="longitudinal",
                subset_biomarker_ids = 2010:2015)
