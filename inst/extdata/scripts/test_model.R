...
#par_tab$biomarker_group <- 1
#par_tab$stratification <- NA
#antibody_data$measurement <- antibody_data$measurement + rnorm(nrow(antibody_data),0,1)
...
res <- serosolver(par_tab, antibody_data, NULL,
                  ...
                  mcmc_pars=c(adaptive_iterations=20000, iterations=50000,proposal_ratio=2,thin_inf_hist=100),verbose=TRUE,verbose_dev = TRUE,
                  start_level = "none",
                  data_type=2)
#Rprof(NULL)
...
chains <- load_mcmc_chains(getwd(),par_tab,burnin=20000)
...
plot_model_fits(chains$theta_chain,chains$inf_chain,antibody_data=antibody_data,individuals = 1:25,settings=res$settings,orientation="longitudinal",known_infection_history = example_inf_hist,data_type = 2,
