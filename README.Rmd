...
par_tab <- check_par_tab(par_tab,TRUE,1,example_antigenic_map$inf_times)
...
res <- run_MCMC(par_tab, antibody_data, example_antigenic_map, start_inf_hist=example_inf_hist,
                filename="test", prior_version=2,
                mcmc_pars=c(adaptive_period=10000, iterations=30000, 
                            inf_propn=0.1,hist_sample_prob=0.5,swap_propn=0.5,
                            save_block=100,thin=10,thin_hist=100,hist_opt=1),
...
n_alive <- get_n_alive_group(antibody_data,example_antigenic_map$inf_times,melt_dat=TRUE)
n_alive$j <- match(n_alive$j,example_antigenic_map$inf_times)

plot_infection_history_chains_time(inf_chain,0000,n_alive=NULL)
plot_attack_rates_pointrange(infection_histories = inf_chain,
                         antibody_data = antibody_data,possible_exposure_times=example_antigenic_map$inf_times,
                         n_alive=NULL,true_ar=all_simulated_data$attack_rates)
