---
output: github_document
---

<!-- Modified by an AI assistant on 2026-09-17 using GPT-5. Updated the worked
data_type example to use character observation-model labels. -->

<!-- README.md is generated from README.Rmd. Please edit that file -->

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  out.width = "100%",
  dpi = 300,
  eval=TRUE
)
library(tidyverse)
```
# serosolver

<img src="man/figures/logo.png" align="right" width="250" alt="serosolver logo">

[![Project Status: Active – The project has reached a stable, usable state and is being actively developed.](https://www.repostatus.org/badges/latest/active.svg)](https://www.repostatus.org/#active)

`serosolver` uses a hierarchical model with a custom Markov chain Monte Carlo sampler to simultaneously infer antibody kinetics and infection histories from cross-sectional or longitudinal serological data. `serosolver` is a [time-since-infection serodynamics model](https://www.sciencedirect.com/science/article/pii/S1755436524000677), meaning that infection times are back-calculated from one or more antibody measurements through an antibody kinetics model. `serosolver` can be used to infer infection timings during a study period using longitudinal measurements against a single antigen, or lifetime infection histories using multi-antigen serology panels. The package and model are described by Hay _et al._ [here](https://doi.org/10.1371/journal.pcbi.1007840).

<br clear="right">

## New features
The current version of `serosolver` includes the following new features.
<details>
  <summary>New features:</summary>
  
* Generalisation to multiple biomarker types per sample (e.g., antibody titre and avidity)
* Support for continuous and discrete observations (e.g., ELISA data and HAI titres)
* Stratification by demographic variables
* Fixing infection states during fitting
* Fixing or estimating starting titres
* Improved user interface
</details>

## Installation

Install the development version of serosolver from [GitHub](https://github.com/seroanalytics/serosolver):

```{r installation,eval=FALSE}
remotes::install_github("seroanalytics/serosolver")
library(serosolver)
```

Note that `serosolver` has been overhauled relative to published version. Please use the `published` branch to ensure continued compatibility with older projects.

```{r, eval=FALSE}
remotes::install_github("seroanalytics/serosolver", ref = "published")
```


## Dependencies

A working C++17 compiler is needed. The package uses `Rcpp`, `RcppArmadillo`, and `RcppParallel`.

```{r package_installation, eval=FALSE}
required_packages <- c(
  "data.table", "ggplot2", "dplyr", "tidyr", "Rcpp", "coda",
  "doParallel", "doRNG", "foreach", "Matrix", "MASS", "reshape2",
  "tibble", "RcppArmadillo", "RcppParallel"
)

additional_packages <- c(
  "remotes", "devtools", "plyr", "tidyverse"
)

install.packages(c(required_packages, additional_packages))
```

## Resources

Read the [guide][vignette-doc] to set up and run a simple implementation with a simulation model.

Additional vignettes:

* [Longitudinal case study][c1-vignette-doc]: estimating infection timings using longitudinal data, with an example from influenza A/H1N1p in Hong Kong
* [Cross-sectional case study][c2-vignette-doc]: estimating life-course infection histories from multi-strain serology, with an example from the [Fluscape cohort](https://pubmed.ncbi.nlm.nih.gov/26875566/)
* [Advanced features][advanced-vignette-doc]: examples of fixed infection states, starting antibody levels, measurement offsets, exponential waning, multiple biomarker groups, and prior version 1
* [Demographic variables and covariates][demographics-vignette-doc]: estimating demographic differences in antibody kinetics and infection-history priors, with an example of variant-stratified parameters
* [Naming conventions][naming-vignette-doc]: the current names for datasets, variables, and model inputs
* [Frequently asked questions][faq-vignette-doc]: common questions about data, model fitting, diagnostics, and troubleshooting

[r-project]: http://cran.r-project.org
[vignette-doc]: https://seroanalytics.github.io/serosolver/articles/serosolver-guide.html
[c1-vignette-doc]: https://seroanalytics.github.io/serosolver/articles/cs1_hong_kong.html
[c2-vignette-doc]: https://seroanalytics.github.io/serosolver/articles/cs2_vignette.html
[advanced-vignette-doc]: https://seroanalytics.github.io/serosolver/articles/advanced_features.html
[demographics-vignette-doc]: https://seroanalytics.github.io/serosolver/articles/demographics_covariates.html
[naming-vignette-doc]: https://seroanalytics.github.io/serosolver/articles/naming_convention.html
[faq-vignette-doc]: https://seroanalytics.github.io/serosolver/articles/faq.html

## Example

This is a basic example of simulating some serological data and fitting the model using the MCMC framework.

```{r example_setup, message=FALSE,warning=FALSE}
library(serosolver)

## Load in example parameter values and antigenic map
data(example_par_tab)
data(example_antigenic_map)
data(example_antibody_data)
data(example_inf_hist)

## Check the dataset and model control table for errors
example_antibody_data <- check_data(example_antibody_data)
example_par_tab <- check_par_tab(example_par_tab)
```
```{r example_plot, message=FALSE,warning=FALSE, fig.width=6,fig.height=5}
plot_antibody_data(example_antibody_data,example_antigenic_map$inf_times,n_indivs=1:5,infection_histories=example_inf_hist)
```

```{r example_mcmc, message=FALSE,warning=FALSE,eval=TRUE}
## Run serosolver
readme_mcmc_dir <- file.path("inst", "extdata", "readme", "chains")
dir.create(readme_mcmc_dir, recursive = TRUE, showWarnings = FALSE)
output <- serosolver::serosolver(example_par_tab, example_antibody_data, antigenic_map=example_antigenic_map,
                filename=file.path(readme_mcmc_dir, "readme"), n_chains=3,parallel=TRUE,data_type="continuous",
                mcmc_pars=c(adaptive_iterations=10000, iterations=50000),verbose=TRUE)
```
```{r example_model_fits, message=FALSE,warning=FALSE,eval=TRUE}
output$plot_fits_cross_sectional
```

## AI declaration

AI assistance was used during development of this package for documentation
drafting, repository audits, code review, and selected implementation edits.
Most of this assistance was provided using OpenAI's GPT-5 model.
The package author is responsible for reviewing and approving all changes.
The core of `serosolver` remains the same as the published version; new
advanced features were developed and implemented manually, with some
AI assistance used to align complex coding workflows.
