...
mcmc_pars <- c("iterations"=1000000,"adaptive_period"=7500000, "burnin"=500000,
               "hist_sample_prob"=0.5,"thin"=50,"thin_hist"=200,"swap_propn"=0.5,
  ...
  foreach(i=1:3)%do%{
    for(type in c("L","CS")){
      ...
      tryCatch( res <- run_MCMC(par_tab = par_tab, titre_dat = merge(titre_dat,ages, by = "individual"),
                                ...
                                mvr_pars = NULL, start_inf_hist = NULL, filename=paste("chains/", filename,"_1_",sep=""),
     ...
     tryCatch( res <- run_MCMC(par_tab = par_tab, titre_dat = merge(titre_dat,ages, by = "individual"),
                      antigenic_map = fit_dat, mcmc_pars = mcmc_pars,
                      mvr_pars = NULL, start_inf_hist = NULL, filename=paste("chains/", filename,"_2_",sep=""),
                      CREATE_POSTERIOR_FUNC = create_posterior_func_fast, CREATE_PRIOR_FUNC = NULL,
                      version = 2,
                      fast_version = TRUE),
      error = function(e) write.table(filename, paste(filename,".txt",sep="")))
