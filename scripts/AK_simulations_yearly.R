...
if(SIM){
  ...
  dat <- simulate_data(parTab,1,n_indiv,strainIsolationTimes,
                       samplingTimes, nsamp,antigenicMap, 0,0,1,40) # Age 0-10
...
mcmcPars <- c("iterations"=1e4,"popt"=0.44,"opt_freq"=1000,"thin"=100,"adaptive_period"=1000,
...
res <- run_MCMC(startTab, data, mcmcPars, filename="tests/test",create_post_func, mvrPars, NULL, 0.2, antigenicMap, ages, startInfHist=NULL)

generate_all_plots(outputDir, mcmcPars["adaptive_period"], res$chain_file, res$history_file,
                   data, antigenicMap, parTab, ages, 10, 100, "tests/testing")
