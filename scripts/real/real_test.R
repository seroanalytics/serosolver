...
startPar <- c(2,2,0.05,1,0.1,0.01,8,1,0.75,4.5)
mcmcPars <- c("iterations"=500000,"popt"=0.44,"popt_hist"=0.44,"opt_freq"=2000,"thin"=10,"adaptive_period"=150000,
              "save_block"=100,"thin2"=100,"histSampleProb"=1,"switch_sample"=2, "burnin"=20000, 
...
p1 <- ggplot(quantHist[quantHist$individual %in% seq(1,9,by=1),]) + 
  ...
  geom_line(data=startHistProfiles[startHistProfiles$individual %in% seq(1,9,by=1),],aes(x=as.integer(variable),y=value), col="red") +
  facet_wrap(~individual, scales="free_y",ncol=3) + theme_bw() + xlab("Circulation year") + ylab("Cumulative number of infections")
...
generate_all_plots(getwd(), 200000, res$chain_file, res$history_file,
                            titreDat, fit_dat, parTab, ages, nIndiv=5,nSamp=100,
                               filename=filename,thin=10)
