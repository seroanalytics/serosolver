...
case_study_bundles <- load_vignette_chain_bundle(
  vignette_dir, "cs1_hong_kong.rds"
)
```
...
if (!is.null(case_study_bundles[["prior"]])) {
  prior_dir <- materialize_vignette_chain_fit(
    case_study_bundles[["prior"]], tempfile("serosolver_cs1_prior_"), "prior"
  )
}
prior_n_chains <- 3
prior_mcmc_pars <- c(
...
if (!is.null(case_study_bundles[["main"]])) {
  mcmc_dir <- materialize_vignette_chain_fit(
    case_study_bundles[["main"]], tempfile("serosolver_cs1_main_"), "hong_kong"
  )
  mcmc_file <- file.path(mcmc_dir, "hong_kong")
}
...
if (!is.null(case_study_bundles[["simulation"]])) {
  sim_mcmc_dir <- materialize_vignette_chain_fit(
    case_study_bundles[["simulation"]], tempfile("serosolver_cs1_sim_"), "simulated"
  )
  sim_mcmc_file <- file.path(sim_mcmc_dir, "simulated")
}
