...
<!-- Modified by an AI assistant on 2026-09-16 using GPT-5. Adapted the student practical into a package vignette while retaining its scientific explanation and code comments, and extended the simulation-recovery section with a runnable MCMC fit and recovery comparisons. -->
...
The assumptions of the fitted model are controlled through the parameter table. `serosolver` allows particular parameters to be estimated or fixed, and we recommend iterative comparison of model structures when identifiability or convergence is a concern.
...
## `simulate_data()` returns the parameter table with derived stratification
## rows already added; serosolver adds these rows when fitting the data.
