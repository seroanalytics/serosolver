...
<!-- Modified by an AI assistant on 2026-09-16 using GPT-5. Adapted the student practical into a package vignette while retaining its scientific explanation and code comments. -->
...
```{r data-plot, fig.height=5, fig.width=8}
...
check_par_tab(par_tab, mcmc = TRUE, possible_exposure_times = possible_exposure_times)
```
...
```{r antibody-model, fig.height=5, fig.width=7}
plot_antibody_model(
  pars = par_tab,
...
prior_dir <- "prior_mcmc_chains_hong_kong"
...
prior_res <- serosolver(
  ...
  possible_exposure_times = possible_exposure_times,
  filename = file.path(prior_dir, "prior"),
  ...
  n_chains = 3,
  parallel = TRUE,
  mcmc_pars = c(adaptive_iterations = 20000, iterations = 50000),
  verbose = TRUE
)
...
## Fit the serosolver model.
mcmc_dir <- "mcmc_chains_hong_kong"
dir.create(mcmc_dir, showWarnings = FALSE)
...
```{r load-chains, eval=FALSE, message=FALSE, warning=FALSE}
...
The simulated data can then be fitted using the same `serosolver()` call as above, with a different output filename. The returned infection histories, attack rates, and antibody predictions can be compared with `simulated_data$infection_histories`, `simulated_data$attack_rates`, and the parameter values in `sim_par_tab`.
