...
melted_dat <- plyr::ddply(melted_dat,.(individual,virus,samples),function(x) cbind(x,"run"=1:nrow(x)))
...
all_chains <- load_mcmc_chains(location=chain_path,thin=1,burnin=100000,
...
all_chains <- load_mcmc_chains(location=chain_path,thin=10,burnin=100000,
...
## Read in the MCMC chains
## Note that `thin` here is in addition to any thinning done during the fitting
sim_all_chains <- load_mcmc_chains(location=chain_path,thin=1,burnin=100000,
                               par_tab=par_tab,unfixed=FALSE,convert_mcmc=TRUE)
