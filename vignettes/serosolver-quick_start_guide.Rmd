...
<!-- Modified by an AI assistant on 2026-09-11 using GPT-5. Updated installation instructions from devtools::install_github() to pak::pak(). -->
...
library(dplyr) ## Used for most data manipulation
library(doParallel) ## Used to run MCMC chains in parallel
library(data.table) ## Used to manipulate large MCMC chain objects
...
output <- serosolver(example_par_tab, antibody_data, antigenic_map=antigenic_map,
                filename="readme", n_chains=3,parallel=TRUE,
                mcmc_pars=c(adaptive_iterations=2000, iterations=5000),verbose=TRUE)
...
Install `serosolver` using the `pak` package:
...
More details on formatting a serological dataset for use in `serosolver` is described in \<\<PLACEHOLDER\>\>.
...
par_tab[par_tab$names %in% c("infection_model_prior_shape1","infection_model_prior_shape1"),"stratification"] <- "population_group"
```
...
There are a few ways the user can implement this depending on their desired model (e.g., should covariates be static over the study period or change with e.g., age?). \<\<PLACEHOLDER\>\> provides a detailed vignette on the model structure and how to implement covariate data in `serosolver`.
...
`serosolver` estimates an infection state for each element of the vector `possible_exposure_times`, which is either passed directly or extract automatically from `antigenic_map$inf_times`. The range and entries of these vectors therefore determines the model time period and resolution of the infection history model. If `antigenic_map` is specified, then you can typically leave `possible_exposure_times` as `NULL`. If you aren't using an antigenic map, then you must specify `possible_exposure_times`.
