---
title: '_serosolver_ quick start guide'
output: 
...
2.  Formatting and inputting a serological data set
3.  Specifying model inputs

-   Antibody kinetics model parameters
...
```{r, eval=TRUE}
packages <- c("ggplot2","plyr","dplyr","data.table")
```
...
chain1 <- chain1[chain1$samp_no >= (mcmc_pars["adaptive_iterations"]+mcmc_pars["burnin"]),]
...
# Remove adaptive period and burn-in
  inf_chain <- inf_chain[inf_chain$samp_no >= (mcmc_pars["adaptive_iterations"]+mcmc_pars["burnin"]),]
