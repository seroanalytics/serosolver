...
cache_candidates <- c(
  file.path("..", "inst", "extdata", "quick_start"),
  file.path("inst", "extdata", "quick_start"),
  system.file("extdata", "quick_start", package = "serosolver")
)
cache_candidates <- cache_candidates[dir.exists(cache_candidates)]
if (!length(cache_candidates)) {
  stop("The pre-cached quick-start MCMC chains could not be found.")
}
...
```{r,echo=FALSE}
plot_antibody_model(pars=example_par_tab,antigenic_map=example_antigenic_map,times=example_antigenic_map$inf_times[1:25])
```
