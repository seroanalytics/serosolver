...
# Read in and generate the antigenic map to read strain relationships from
  antigenicMap <- read.csv("fonville_map_approx.csv",stringsAsFactors=FALSE)
...
```{r, eval=FALSE}
# run MCMC  
  res <- run_MCMC(par_tab = startTab, titre_dat = titreDat, antigenic_map = fit_dat, 
                  start_inf_hist = startInf, CREATE_POSTERIOR_FUNC = create_posterior_func,
                  version = 1)
```
