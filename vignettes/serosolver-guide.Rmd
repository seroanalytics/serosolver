...
knitr::opts_chunk$set(
  ...
  fig.width = 8,
  fig.height = 6,
  fig.path = file.path(tempdir(), "serosolver_guide_figures/")
)
...
cache_candidates <- c(
  file.path("..", "inst", "extdata", "guide"),
  file.path("inst", "extdata", "guide"),
  system.file("extdata", "guide", package = "serosolver")
)
cache_candidates <- cache_candidates[dir.exists(cache_candidates)]
if (!length(cache_candidates)) {
  stop("The pre-cached guide MCMC chains could not be found.")
}
mcmc_dir <- cache_candidates[[1]]
...
mcmc_pars <- c(adaptive_iterations=2000, iterations=5000,
               thin=5, thin_inf_hist=5, save_block=1000)
```
...
Alternatively, users can model waning as exponential decay *on the log scale* (so exponential-on-exponential on the natural scale) by setting `exponential_waning=TRUE` in the `serosolver` call:
...
If the default `exponential_waning=FALSE` is assumed, then the range of the waning parameters (lower and upper bounds) should be 0-1. Otherwise, if `exponential_waning=TRUE`, then the parameters should be assumed to be positively bounded.

#### 2.2.3 Cross-reactivity
...
If the default `exponential_waning=FALSE` is assumed, then the range of the cross-reactivity parameters (lower and upper bounds) should be 0-1. Otherwise, if `exponential_waning=TRUE`, then the parameters should be assumed to be positively bounded.
...
Most applications of `serosolver` involve fitting to antibody titre or concentration data. Make a note of whether your data are continuous or discrete, and the upper and lower limits of detection of the assay. Continuous data can take values across a range, whereas discrete data take separate values such as integer titre categories. These limits and the measurement type will need to be specified in the model using the `par_tab` object. `serosolver` also supports multiple measurement types per sample, for example, measuring both antibody titre and avidity. These observation types are distinguished by the `biomarker_group` variable -- all observations within a `biomarker_group` are underpinned by their own antibody kinetics model parameters, but arising from the same individual-level infection histories.
...
These features require additional data preparation and model decisions. They are explained in detail in the [advanced features vignette](ADVANCED_FEATURES_VIGNETTE_LINK) and the relevant case-study material.
...
load(file.path(mcmc_dir, "guide_serosolver_settings.RData"))
```
