...
vignette: |
  %\VignetteIndexEntry{Serosolver Vignette} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
## Choose wd to run anaylsis
setwd("~/Documents/serosolver/vignettes")
```
...
# inputDat <- read.csv(file = "case_study_1/data/HKdata_h1n1_vac.csv",header = TRUE)
...
antigenic_coords <- read.csv("case_study_1/data/fonville_map_approx.csv",stringsAsFactors=FALSE)
...
```{r Run mcmc 1, eval=FALSE, include=TRUE}
...
res <- foreach(x = filenames) %dopar% {
  ...
  res <- run_MCMC(par_tab = start_tab, 
                  ...
                  mcmc_pars = c("iterations"=500000,"adaptive_period"=100000,"thin"=10,"thin_hist"=1000,"save_block"=1000,
                                "inf_propn"=1, "hist_sample_prob"=1,
                                "hist_switch_prob"=0.8, "year_swap_propn"=1),
                  filename = paste0("chains_main/",x), 
...
all_chains <- load_mcmc_chains(location="case_study_1/chains_main",thin=1,burnin=10000,
...
all_chains <- load_mcmc_chains(location="case_study_1/chains_main/",thin=10,burnin=10000,
...
## Read in the MCMC chains
## Note that `thin` here is in addition to any thinning done during the fitting
sim_all_chains <- load_mcmc_chains(location="case_study_1/chains_sim",thin=1,burnin=10000,
                                   par_tab=par_tab,unfixed=FALSE,convert_mcmc=TRUE)
