...
res <- foreach(x = filenames, .packages = 'serosolver') %dopar% {
  ...
  res <- run_MCMC(par_tab = start_tab, 
                  ...
                  mcmc_pars = c("iterations"=50000,"adaptive_period"=10000,"thin"=10,"thin_hist"=100,
                                ...
                                "year_swap_propn"=1),
                  filename = paste0("inst/extdata/chains_main/",x), 
...
all_chains <- load_mcmc_chains(location=paste0(chain_path,"chains_main/"),thin=1,burnin=1000,
...
all_chains <- load_mcmc_chains(location=paste0(chain_path,"chains_main/"),thin=10,burnin=1000,
...
res <- foreach(x = filenames, .packages = "serosolver") %dopar% {
  ...
  res <- run_MCMC(par_tab = start_tab, 
                  ...
                  filename = paste0("inst/extdata/chains_sim/",x), 
                  CREATE_POSTERIOR_FUNC = create_posterior_func_fast, 
                  version = prior_version,
                  fast_version=TRUE)
}
...
## Read in the MCMC chains
## Note that `thin` here is in addition to any thinning done during the fitting
sim_all_chains <- load_mcmc_chains(location=paste0(chain_path,"chains_sim/"),thin=1,burnin=1000,
                                   par_tab=par_tab,unfixed=FALSE,convert_mcmc=TRUE)
