...
input_dat_unvac <- read.csv(file = input_dat_path, header = TRUE)
# vaccinated
...
indivs <- unique(input_dat_unvac$individual) #all individuals

# Subset data for indivs
titre_dat_unvac <- input_dat_unvac[input_dat_unvac$individual %in% indivs,c("individual","virus","titre","samples","DOB")]
titre_dat_unvac$individual <- match(titre_dat_unvac$individual, indivs)

titre_dat_unvac <- unique(titre_dat_unvac)
titre_dat_unvac <- plyr::ddply(titre_dat_unvac,.(individual,virus,samples),function(x) cbind(x,"run"=1:nrow(x),"group"=1))
print(head(titre_dat_unvac))

strain_isolation_times <- seq(sample_years[1]*resolution, sample_years[4]*resolution, by=1)
```
...
par_tab[par_tab$names %in% c("tau","sigma1","sigma2"),"fixed"] <- 1 #mu, tau, sigma1, and sigma2 are fixed
par_tab[par_tab$names %in% c("tau","sigma1","sigma2"),"values"] <- 0 # set these values to 0
```
## Summary
...
```
```{r Run mcmc setup, eval=FALSE, include=TRUE}
...
res <- foreach(x = filenames, .packages = c('serosolver','data.table','plyr')) %dopar% {
  ...
  while(!is.finite(start_prob)){
    ...
    start_inf <- setup_infection_histories_titre(titre_dat_unvac, strain_isolation_times, space=3,titre_cutoff=4)
  ...
  res <- run_MCMC(par_tab = start_tab, 
                  ...
                  mcmc_pars = c("iterations"=20000,"popt"=0.44,"popt_hist"=0.44,
                                "opt_freq"=1000,"thin"=1,"adaptive_period"=5000, 
...
all_chains <- load_mcmc_chains(location=chain_path_real,thin=10,burnin=500000,
...
res <- foreach(x = filenames, .packages = c('serosolver','data.table','plyr')) %dopar% {
  ...
  while(!is.finite(start_prob)){
    ...
    start_tab <- generate_start_tab(par_tab)
    ## Generate starting infection history
    start_inf <- setup_infection_histories_titre(sim_titre_dat, strain_isolation_times, space=3,titre_cutoff=4)
    start_prob <- sum(model_func(start_tab$values, start_inf)[[1]])
  }
  ...
  res <- run_MCMC(par_tab = start_tab, 
                  ...
                  mcmc_pars = c("iterations"=2000000,"popt"=0.44,"popt_hist"=0.44,
                                "opt_freq"=1000,"thin"=10,"adaptive_period"=500000, 
                                "save_block"=1000, "thin_hist"=100,"hist_sample_prob"=1,
                                "switch_sample"=2, "burnin"=0, "inf_propn"=0.5, 
                                "move_size"=3,"hist_opt"=1,"swap_propn"=0.5,
                                "hist_switch_prob"=0.5,"year_swap_propn"=1),
